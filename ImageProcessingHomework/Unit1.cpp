...
{
	//Initial Picture 10
	InitialPicture10(imPicture9,imPicture10);
	//Initial Picture 8
...
void GrayPalette(TImage *imPicture, TImage *imColorModel)
{
	...
	for (j = 0; j < imPicture->Picture->Height; j++)
		...
		for (i = 0; i < imPicture->Picture->Width; i++)
			bPtrImageRow[i] = (unsigned char)((0.299 * bPtrImageRowTemp[3*i +2] +
												0.587 * bPtrImageRowTemp[3*i+1] +
												0.114 * bPtrImageRowTemp[3*i])+ 0.5);
		}
...
void __fastcall TForm1::tkbRColorModelChange(TObject *Sender)
{
	...
	//Redraw Picture8
	ReDrawPicture8(imPicture7,imPicture8,imRColorModel);
}
//---------------------------------------------------------------------------
void __fastcall TForm1::tkbGColorModelChange(TObject *Sender)
{
	...
	//Redraw Picture8
	ReDrawPicture8(imPicture7,imPicture8,imGColorModel);
...
void __fastcall TForm1::tkbBColorModelChange(TObject *Sender)
{
	...
	//Redraw Picture8
	ReDrawPicture8(imPicture7,imPicture8,imBColorModel);
}
//---------------------------------------------------------------------------
		  ...
		  TShiftState Shift, int X, int Y)
{
	...
	//Redraw Picture8
	ReDrawPicture8(imPicture7,imPicture8,imYUVColorModel);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void ReDrawPicture8(TImage *imPicture7, TImage *imPicture8,TImage* imColorModel)
{
	if (iPositionX == 0 && iPositionY == 0)
		{
		ShowMessage("請選顏色");
		return;
		}
	//Get RGB from YUVColorModel
	bPtrImageRow = (Byte *)imColorModel->Picture->Bitmap->ScanLine[iPositionY];
	int	iR = bPtrImageRow[3*iPositionX +2];
	int iG = bPtrImageRow[3*iPositionX +1];
	int iB = bPtrImageRow[3*iPositionX];
...
//---------------------------------------------------------------------------
void __fastcall TForm1::imRColorModelMouseDown(TObject *Sender, TMouseButton Button,
		  TShiftState Shift, int X, int Y)
{
	//Set Global variable
	iPositionX = X;
...
//---------------------------------------------------------------------------
void __fastcall TForm1::imGColorModelMouseDown(TObject *Sender, TMouseButton Button,
		  TShiftState Shift, int X, int Y)
{
	//Set Global variable
	iPositionX = X;
...
//---------------------------------------------------------------------------
void __fastcall TForm1::imBColorModelMouseDown(TObject *Sender, TMouseButton Button,
		  TShiftState Shift, int X, int Y)
{
	//Set Global variable
	iPositionX = X;
...
//---------------------------------------------------------------------------
void InitialPicture10(TImage *imPicture,TImage *imPicture10)
{
	//Set Canvas
	imPicture10->Width = imPicture->Width;
	imPicture10->Height = imPicture->Height;
	imPicture10->Picture->Bitmap->Width = imPicture->Picture->Bitmap->Width;
	imPicture10->Picture->Bitmap->Height = imPicture->Picture->Bitmap->Height;
	imPicture10->Picture->Bitmap->PixelFormat = pf24bit;
	...
	for (j = 0; j < imPicture->Picture->Height; j++)
		{
		bPtrImageRow = (Byte *)imPicture10->Picture->Bitmap->ScanLine[j];
		bPtrImageRowTemp = (Byte *)imPicture->Picture->Bitmap->ScanLine[j];
		for (i = 0; i < imPicture->Picture->Width; i++)
			{
			iR = bPtrImageRowTemp[3*i +2];
			iG = bPtrImageRowTemp[3*i +1];
			iB = bPtrImageRowTemp[3*i];
...
void __fastcall TForm1::tkbChangeYChange(TObject *Sender)
{
	int iR = 0, iG = 0, iB = 0;
	int iY = 0, iU = 0, iV = 0;
	int iC = 0, iD = 0, iE = 0;

	for (j = 0; j < imPicture10->Picture->Height; j++)
		{
		bPtrImageRow = (Byte *)imPicture10->Picture->Bitmap->ScanLine[j];
		bPtrImageRowTemp = (Byte *)imPicture9->Picture->Bitmap->ScanLine[j];
		for (i = 0; i < imPicture10->Picture->Width; i++)
			{
			iR = bPtrImageRowTemp[3*i +2];
			iG = bPtrImageRowTemp[3*i +1];
			iB = bPtrImageRowTemp[3*i];
